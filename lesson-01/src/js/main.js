...
function renderTaskList(items) {
    ...
    for (const item of items) {
        ...
        const checked = item.done ? '(done)' : '(todo)';
        html += `
        <li class="${status}" data-title="${item.title}">
...
addMessage('This message was appened with createElement');
// --------------------------------------------------
...
// 4. Stretch goals:
//    - Display a checkbox next to each task to represent done/todo 
